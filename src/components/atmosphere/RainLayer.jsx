...
export default function RainLayer({ scrollYProgress }) {
  ...
  useEffect(() => {
    setRaindrops(Array.from({ length: 150 }, (_, i) => ({
      id: i,
  ...
  return (
    <motion.div className="absolute inset-0 pointer-events-none overflow-hidden" style={{ opacity: rainOpacity }}>
      {raindrops.map(drop => (
        <motion.div key={drop.id} className="absolute w-px h-10 bg-slate-300/40"
