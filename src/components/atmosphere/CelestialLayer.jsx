...
export default function CelestialLayer({ scrollYProgress }) {
  ...
  return (
    <>
      <motion.div className="absolute left-[15%] w-32 h-32 rounded-full"
        style={{
          y: sunY, opacity: sunOpacity, scale: sunScale,
          background: 'radial-gradient(circle, rgba(255,237,160,0.8) 0%, rgba(255,200,100,0.4) 50%, transparent 70%)',
          filter: 'blur(2px)'
        }}
      />
      <motion.div className="absolute right-[20%] w-20 h-20 rounded-full bg-slate-200/60"
        style={{ y: moonY, opacity: moonOpacity, filter: 'blur(1px)' }}
      >
