  ...
  return (
    <>
      <motion.div className="absolute inset-0 pointer-events-none"
        style={{ opacity: morningCloudsOpacity, x: cloudDrift }}>
      ...
      <motion.div className="absolute inset-0 pointer-events-none"
        style={{ opacity: afternoonCloudsOpacity, x: cloudDrift }}>
        <div className="absolute top-[25%] left-[20%] w-52 h-20 bg-slate-400/30 rounded-full blur-xl" />
        <div className="absolute top-[40%] right-[25%] w-60 h-24 bg-slate-400/25 rounded-full blur-xl" />
      </motion.div>

      <motion.div className="absolute inset-0 pointer-events-none" style={{ opacity: stormCloudsOpacity }}>
        {Array.from({ length: 5 }).map((_, i) => (
          <motion.div key={`storm-${i}`} className="absolute w-80 h-32 bg-slate-700/50 rounded-full blur-2xl"
            style={{ top: `${10 + i * 12}%`, left: `${i * 20}%` }}
